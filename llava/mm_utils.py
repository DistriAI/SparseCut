...
def process_anyres_image(image, processor, grid_pinpoints):
    ...
    #  较大的图像先通过选择最合适的比例，然后通过resize和pad 得到高分辨率图像
    ...
    # 这里可能需要写一下高分辨率图像token的reorder

    return torch.stack(image_patches, dim=0)


def process_anyres_image4hyres(image, processor,
                         num_crops=None,
                         grid_pinpoints=None,
                         ):
    """
    ...
    if type(grid_pinpoints) is list:
        ...
        possible_resolutions = ast.literal_eval(grid_pinpoints)  # 安全的字符串转list
    best_resolution = select_best_resolution(image.size, possible_resolutions)
    ...
    image_patches = [processor.preprocess(image_patch, return_tensors='pt')['pixel_values'][0]
                     for image_patch in image_patches]
    # print('shape in process_anyres_image4hyres', torch.stack(image_patches, dim=0).shape)
...
def process_images(images, image_processor, model_cfg):
    ...
    # print('model_cfg.image_grid_points', model_cfg.image_grid_pinpoints)
    ...
    if image_aspect_ratio == 'pad':
        ...
        for image in images:
            ...
            new_images.append(image)
    elif image_aspect_ratio == "hyres":
        for image in images:
            image, _  = process_anyres_image4hyres(image, image_processor, num_crops=4)
            # 此处还没整好
            new_images.append(image)
    else:
